...
static void memfs_release(struct fsal_export *exp_hdl)
{
			...
			fsal_fsinfo_options_t option)
{
	...
	info = memfs_staticinfo(exp_hdl->fsal);
...
/* memfs_export_ops_init
 * overwrite vector entries with the methods that we support
 */

void memfs_export_ops_init(struct export_ops *ops)
{
	ops->release = memfs_release;
	ops->lookup_path = memfs_lookup_path;
	ops->extract_handle = extract_handle;
	ops->create_handle = memfs_create_handle;
				     ...
				     const struct fsal_up_vector *up_ops)
{
	...
	myself = gsh_calloc(1, sizeof(struct memfs_fsal_export));
	...
	memfs_export_ops_init(&myself->export.exp_ops);
