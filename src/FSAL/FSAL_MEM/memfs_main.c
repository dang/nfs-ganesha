...
/* MEMFS FSAL module private storage
 */

/* defined the set of attributes supported with POSIX */
#define MEMFS_SUPPORTED_ATTRIBUTES (				\
	ATTR_TYPE     | ATTR_SIZE     |				\
...
struct mem_fsal_module {
	...
	/* memfsfs_specific_initinfo_t specific_info;  placeholder */
};

const char memname[] = "MEM";

/* filesystem info for MEMFS */
static struct fsal_staticfsinfo_t default_memfs_info = {
	...
	.cansettime = true,
	.homogenous = true,
	.supported_attrs = MEMFS_SUPPORTED_ATTRIBUTES,
...
static struct config_item memfs_items[] = {
	CONF_ITEM_NOOP("placeholder"),
	CONFIG_EOL
};

static struct config_block memfs_block = {
	.dbus_interface_name = "org.ganesha.nfsd.config.fsal.mem",
	.blk_desc.name = "MEM",
	.blk_desc.type = CONFIG_BLOCK,
	.blk_desc.u.blk.init = noop_conf_init,
	.blk_desc.u.blk.params = memfs_items,
...
struct fsal_staticfsinfo_t *memfs_staticinfo(struct fsal_module *hdl)
{
...
{
	struct mem_fsal_module *memfs_me =
	...
	memfs_me->fs_info = default_memfs_info;
	...
	display_fsinfo(&memfs_me->fs_info);
	LogFullDebug(COMPONENT_FSAL,
		     "Supported attributes constant = 0x%" PRIx64,
		     (uint64_t) MEMFS_SUPPORTED_ATTRIBUTES);
	LogFullDebug(COMPONENT_FSAL,
		     "Supported attributes default = 0x%" PRIx64,
		     default_memfs_info.supported_attrs);
	LogDebug(COMPONENT_FSAL,
		 "FSAL INIT: Supported attributes mask = 0x%" PRIx64,
		 memfs_me->fs_info.supported_attrs);
...
bool memfs_support_ex(struct fsal_obj_handle *obj)
{
	return true;
}
...
static struct mem_fsal_module MEMFS;
...
{
	int retval;
	struct fsal_module *myself = &MEMFS.fsal;
	...
	myself->m_ops.create_export = memfs_create_export;
	myself->m_ops.init_config = init_config;
	myself->m_ops.support_ex = memfs_support_ex;
}
...
MODULE_FINI void finish(void)
{
	...
	retval = unregister_fsal(&MEMFS.fsal);
	if (retval != 0) {
		LogCrit(COMPONENT_FSAL,
