...
static inline int
memfs_n_cmpf(const struct avltree_node *lhs,
...
static inline int
memfs_i_cmpf(const struct avltree_node *lhs,
		const struct avltree_node *rhs)
{
	struct mem_fsal_obj_handle *lk, *rk;
...
/**
 * @brief Construct the fs opaque part of a memfs nfsv4 handle
 *
 * Given the components of a memfs nfsv4 handle, the nfsv4 handle is
 ...
 *
 * @param[in] pathbuf Full patch of the memfs node
 * @param[in] hashkey a 64 bit hash of the mempath parameter
 *
 * @return The nfsv4 memfs file handle as a char *
 */
				  ...
				  struct display_buffer *pathbuf)
{
	 ...
	 * Ideally we can include entire memfs pathname for guaranteed
	 * uniqueness of memfs handles.
	 */
	...
	/* If there is more space in the opaque handle due to a short memfs
	 * path ... zero it.
	 */
...
/**
 * @brief Concatenate a number of memfs tokens into a string
 *
 * When reading memfs paths from export entries, we divide the
		  ...
		  struct attrlist *attrs)
{
	...
	memfs_handle_ops_init(&hdl->obj_handle.obj_ops);

	switch (type) {
	case DIRECTORY:
		avltree_init(&hdl->mh_dir.avl_name, memfs_n_cmpf, 0);
		avltree_init(&hdl->mh_dir.avl_index, memfs_i_cmpf, 0);
		hdl->next_i = 2;
...
{
	struct mem_fsal_obj_handle *hdl = NULL;
				 ...
				 bool *eof)
{
	...
	if (seekloc == 0) {
		...
		fsal_copy_attrs(&attrs, &myself->attrs, false);
		if (!cb(".", dir_hdl, &attrs, dir_state, 0)) {
			*eof = false;
			return fsalstat(ERR_FSAL_NO_ERROR, 0);
		}
		seekloc++;
	...
	if (seekloc == 1) {
		...
		if (myself->parent) {
			fsal_prepare_attrs(&attrs, attrmask);
			fsal_copy_attrs(&attrs, &myself->parent->attrs, false);
			if (!cb("..", &myself->parent->obj_handle, &attrs,
				dir_state, 0)) {
				*eof = false;
				return fsalstat(ERR_FSAL_NO_ERROR, 0);
			}
				...
				struct attrlist *attrs_out)
{
	...
	status = mem_create_obj(parent, DIRECTORY, name, attrs_in, new_obj,
				attrs_out);
				  ...
				  bool refresh)
{
	...
	link_content->len = strlen(myself->mh_symlink.link_contents);
				...
				const char *name)
{
	...
	avltree_remove(&myself->avl_n, &parent->mh_dir.avl_name);
	avltree_remove(&myself->avl_i, &parent->mh_dir.avl_index);
	myself->inavl = false;
				...
				const char *new_name)
{
	...
	/* Remove from old dir */
	status = mem_unlink(olddir_hdl, obj_hdl, old_name);
	if (FSAL_IS_ERROR(status)) {
		return status;
	}
			 ...
			 struct io_info *info)
{
	...
	if (!open_correct(my_fd->openflags, openflags)) {
		return fsalstat(ERR_FSAL_NOT_OPENED, 0);
	}
...
fsal_status_t memfs_lookup_path(struct fsal_export *exp_hdl,
				 ...
				 struct attrlist *attrs_out)
{
	struct memfs_fsal_export *myself;
	struct attrlist attrs;
