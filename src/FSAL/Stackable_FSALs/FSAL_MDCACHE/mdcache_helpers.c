		...
		struct fsal_filesystem *fs)
{
	...
	state_hdl_init(&result->fsobj.hdl, result->obj_handle.type,
		       &result->obj_handle);
	result->obj_handle.state = &result->fsobj.hdl;
		  ...
		  mdcache_entry_t **entry)
{
	...
	*entry = NULL;
	...
	oentry = cih_get_by_key_latched(&key, &latch, CIH_GET_WLOCK, __func__,
					__LINE__);
	if (oentry) {
...
static fsal_status_t mdc_add_cache(mdcache_entry_t *mdc_parent,
				   const char *name,
				   struct fsal_obj_handle *sub_handle,
 ...
 * @return FSAL status
 */
fsal_status_t mdcache_lookup_uncached(mdcache_entry_t *mdc_parent,
				      const char *name,
				      mdcache_entry_t **new_entry)
{
...
mdcache_dirent_populate(mdcache_entry_t *dir)
{
	...
	if (!eod && mdcache_param.retry_readdir) {
		...
		/* End of work */
		atomic_set_uint32_t_bits(&dir->mde_flags, MDCACHE_DIR_POPULATED);
