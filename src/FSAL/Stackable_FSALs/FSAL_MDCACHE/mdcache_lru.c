	...
	for (ix = 0; ix < LRU_N_Q_LANES; ++ix, lane = LRU_NEXT(reap_lane)) {
		...
		if (cih_latch_entry(&entry->fh_hk.key, &latch, CIH_GET_WLOCK,
			...
			if (LRU_ENTRY_RECLAIMABLE(entry, refcnt)) {
				 ...
				 * cache_inode_lru_unref(), which released the
...
alloc_cache_entry(mdcache_entry_t **entry)
{
	...
	nentry = pool_alloc(mdcache_entry_pool, NULL);
 ...
 * @param[in] flags  One of LRU_REQ_INITIAL, LRU_REQ_STALE_OK, else LRU_FLAG_NONE
 *
 ...
 * dosen't move things within L2 (only between).
 *
 * @return FSAL status
 */
fsal_status_t
