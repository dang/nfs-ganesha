 ...
 * @return FSAL status
 */
static fsal_status_t mdc_try_get_cached(mdcache_entry_t *mdc_parent,
					const char *name,
					mdcache_entry_t **entry)
{
			    ...
			    const char *name, struct fsal_obj_handle **handle)
{
	...
	*handle = NULL;
	 ...
	 * the FSAL. */
	status = mdc_try_get_cached(mdc_parent, name, &entry);
	...
	if (!FSAL_IS_ERROR(status)) {
		/* Success! */
		goto out;
	} else if (FSAL_IS_ERROR(status) && status.major != ERR_FSAL_STALE) {
		/* Actual failure */
		goto out;
	}
	...
	status = mdcache_lookup_uncached(mdc_parent, name, &entry);
				  ...
				  fsal_readdir_cb cb, bool *eod_met)
{
	...
	*eod_met = false;
	...
	for (; cb_result && dirent_node;
		...
		if (status.major == ERR_FSAL_STALE) {
			status = mdcache_lookup_uncached(directory,
							 dirent->name,
							 &entry);
		}
		if (FSAL_IS_ERROR(status)) {
 ...
 * @return FSAL status
 */
static fsal_status_t mdcache_handle_digest(const struct fsal_obj_handle *obj_hdl,
 ...
 * @return FSAL status
 */
static void mdcache_release(struct fsal_obj_handle *obj_hdl)
{
	mdcache_entry_t *entry =
		container_of(obj_hdl, mdcache_entry_t, obj_handle);
