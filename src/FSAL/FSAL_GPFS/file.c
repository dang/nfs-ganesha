...
gpfs_read2(struct fsal_obj_handle *obj_hdl, bool bypass, struct state_t *state,
	   uint64_t offset, fsal_async_cb done_cb, struct fsal_read_arg
	   *read_arg, void *caller_arg)
{
	int my_fd = -1;
	fsal_status_t status;
	bool has_lock = false;
	...
	if (read_arg->info)
		status = gpfs_read_plus_fd(my_fd, offset,
		...
		status = GPFSFSAL_read(my_fd, offset, read_arg->iov[0].iov_len,
				       read_arg->iov[0].iov_base,
				       &read_arg->read_amount,
