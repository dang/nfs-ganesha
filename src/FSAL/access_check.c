					   ...
					   struct attrlist *p_object_attributes)
{
	...
	if (!p_object_attributes->acl) {
		/* Means that FSAL_ACE4_FLAG_REQ was set, but no ACLs */
		LogFullDebug(COMPONENT_NFS_V4_ACL,
	...
	if (IS_FSAL_ACE4_REQ(v4mask) && missing_access) {
		...
		if ((missing_access &
			...
			LogDebug(COMPONENT_NFS_V4_ACL, "final access denied (EPERM)");
			       ...
			       fsal_accessflags_t *denied)
{
	...
	if (IS_FSAL_ACE4_REQ(access_type) ||
	    (attribs->acl && IS_FSAL_ACE4_MASK_VALID(access_type))) {
