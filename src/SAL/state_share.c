			       ...
			       state_t *state, bool reclaim)
{
	...
	old_entry_share_deny = state_share_get_share_deny(obj->state);
	...
	state_share_update_counter(obj->state, OPEN4_SHARE_ACCESS_NONE,
				   OPEN4_SHARE_DENY_NONE, new_share_access,
				   new_share_deny, true);

	/* Get the updated union of share states of this file. */
	new_entry_share_access = state_share_get_share_access(obj->state);
	new_entry_share_deny = state_share_get_share_deny(obj->state);
	...
	if ((new_entry_share_access != old_entry_share_access)
		...
		if (status != STATE_SUCCESS) {
			...
			state_share_update_counter(obj->state, new_share_access,
						   new_share_deny,
				  ...
				  state_t *state)
{
	...
	/* Update the ref counted share state of this file. */
	state_share_update_counter(obj->state, removed_share_access,
				   ...
				   bool reclaim)
{
	...
	/* Update the ref counted share state of this file. */
	state_share_update_counter(obj->state, old_share_access, old_share_deny,
					      ...
					      enum share_bypass_modes bypass)
{
	...
	atomic_inc_uint32_t(&obj->state->file.anon_ops);
...
void state_share_anonymous_io_done(struct fsal_obj_handle *obj, int share_access)
{
	 ...
	 */
	state_share_update_counter(obj->state, share_access,
				   OPEN4_SHARE_DENY_NONE,
				   OPEN4_SHARE_ACCESS_NONE,
				   OPEN4_SHARE_DENY_NONE, false);
	...
	atomic_dec_uint32_t(&obj->state->file.anon_ops);
}
			       ...
			       bool reclaim)
{
	...
	glist_add_tail(&obj->state->file.nlm_share_list, &state->state_list);
