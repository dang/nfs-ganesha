				    ...
				    enum cb_state cb_state)
{
	...
	if (cb_parms->attr_allowed &&
	    obj->type == DIRECTORY &&
	    obj->state->dir.junction_export != NULL &&
		...
		LogDebug(COMPONENT_EXPORT,
			 ...
			 cb_parms->name,
			 obj->state->dir.junction_export->export_id,
			 obj->state->dir.junction_export->fullpath);

		/* Get a reference to the export and stash it in
		 * compound data.
		 */
		if (!export_ready(obj->state->dir.junction_export)) {
		...
		/* Cross the junction */
		op_ctx->export = obj->state->dir.junction_export;
		op_ctx->fsal_export = op_ctx->export->fsal_export;
