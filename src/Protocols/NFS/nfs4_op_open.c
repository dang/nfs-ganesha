			      ...
			      bool skip_permission)
{
	...
	*new_state = true;
	...
	if (*new_state) {
		state_status = state_add_impl(ostate,
					      STATE_TYPE_SHARE,
					      &candidate_data,
					      owner,
					      &file_state,
...
{
	/* Building a new fh */
	if (!nfs4_FSALToFhandle(false, &data->currentFH, obj, op_ctx->export)) {
		return NFS4ERR_SERVERFAULT;
	}
			   ...
			   bool prerecall)
{
	...
	state_status = state_add_impl(ostate, STATE_TYPE_DELEG,
				      &state_data,
