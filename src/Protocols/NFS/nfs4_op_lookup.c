		   ...
		   struct nfs_resop4 *resp)
{
	...
	if (file_obj->type == DIRECTORY) {
		...
		if (file_obj->state->dir.junction_export != NULL) {
			 ...
			 */
			if (!export_ready(
				file_obj->state->dir.junction_export)) {
				...
				PTHREAD_RWLOCK_unlock(
					&file_obj->state->state_lock);
				goto out;
			}

			get_gsh_export_ref(file_obj->state->dir.junction_export);
			...
			op_ctx->export = file_obj->state->dir.junction_export;
		...
		PTHREAD_RWLOCK_unlock(&file_obj->state->state_lock);
	}
