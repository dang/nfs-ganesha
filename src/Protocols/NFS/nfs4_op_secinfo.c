		    ...
		    struct nfs_resop4 *resp)
{
	...
	PTHREAD_RWLOCK_rdlock(&obj_src->state->state_lock);

	if (obj_src->type == DIRECTORY &&
	    obj_src->state->dir.junction_export != NULL) {
		/* Handle junction */
		struct fsal_obj_handle *obj = NULL;

		/* Try to get a reference to the export. */
		if (!export_ready(obj_src->state->dir.junction_export)) {
			/* Export has gone bad. */
			LogDebug(COMPONENT_EXPORT,
				 "NFS4ERR_STALE On Export_Id %d Path %s",
				 obj_src->state->dir.junction_export->export_id,
				 obj_src->state->dir.junction_export->fullpath);
			res_SECINFO4->status = NFS4ERR_STALE;
			PTHREAD_RWLOCK_unlock(&obj_src->state->state_lock);
			goto out;
		}

		get_gsh_export_ref(obj_src->state->dir.junction_export);

		/* Save the compound data context */
		save_export_perms = *op_ctx->export_perms;
		saved_gsh_export = op_ctx->export;

		op_ctx->export = obj_src->state->dir.junction_export;
